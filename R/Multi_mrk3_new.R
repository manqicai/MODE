...
#'
#' @examples
Multi_mrk3_new <- function(DM_df = DM_df,eQTM_df = eQTM_df,DE_df = DE_df,padjust_method = "bonferroni",
  ...
  if(filter_type == "prop"){
    ...
    com1 <- right_join(com1,sub_signal)
  }
  ...
  # Third Adjust pvalue & filter DMR and select top markers per cell type
  ...
  cat("New first alg3: Number of markers per ct: ", sapply(output_mrk,length))
  ...
  #               bench = NULL))
  # }else{
  # cat("New first alg3: Number of markers: ", nrow(sig))
