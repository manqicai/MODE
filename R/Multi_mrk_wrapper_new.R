...
#'
#' @examples
Multi_mrk_wrapper_new <- function(com_params,my_DM_df,my_eQTM_df,my_DE_df,my_DM_df_onesided,
                              ...
                              ,padjust_method = "bonferroni",sc_signal_oneside, sc_signal_twosided,
                              alg1only = FALSE){
  ...
  cl <- makeCluster(ncore, outfile="D:/Manqi/Meeting/20230220_23/log.txt")
  registerDoSNOW(cl)
  clusterCall(cl, function(x) .libPaths(x), .libPaths())
  res <- foreach(rownum = 1:nrow(com_params), .export=c("DNAm_minfi_deconv_general","MyestimateCellCounts",
                                                        "pickCompProbes","projectCellType"),
                 .packages=c("dplyr","reshape2","tidyverse","EnsDeconv","ACAT","sparseMatrixStats","tidyverse",
                             "MIND","minfi","bumphunter","quadprog","EpiSCORE","data.table","readxl"),.options.snow=opts, .errorhandling = 'pass') %dopar% {
    sig_name = paste0(unlist(paste0(colnames(com_params),"_", com_params[rownum,])), collapse = "")
    suppressMessages(source("D:/Manqi/Meeting/20221027/utilities.R"))
    suppressMessages(source("D:/Manqi/Meeting/20221023/utilities.R"))
    source("D:/Manqi/Package/MODE/R/Multi_mrk_new.R")
    source("D:/Manqi/Package/MODE/R/Multi_mrk3_new.R")
    ...
    # suppressMessages(source("D:/Manqi/deconv.ensemble/R/get_metric_wrap.R"))
    # suppressMessages(source("D:/Manqi/Package/MODE/R/sig_benchmarking.R"))

    output <- Multi_mrk_new(DM_df = my_DM_df,eQTM_df = my_eQTM_df,DE_df = my_DE_df,padjust_method =padjust_method,
                        ct_ind = ct_ind,pval_threshold = com_params$pval_threshold[rownum],
                        filter_type = com_params$filter_type[rownum],cutoff_val = com_params$cutoff_val[rownum],DE_type = com_params$DE_type[rownum],n_mrk = com_params$n_mrk[rownum],beta_mtx = beta_mtx,
                        ...
                        sc_signal = sc_signal_twosided,cpg_pergene = com_params$cpg_pergene[rownum],fin_ncpg = com_params$fin_ncpg[rownum],cutoff_val2 = com_params$cutoff_val2[rownum])
    output_onesided <- Multi_mrk_new(DM_df = my_DM_df_onesided,eQTM_df = my_eQTM_df,DE_df = my_DE_df_onesided,padjust_method =padjust_method,
                                 ct_ind = ct_ind,pval_threshold = com_params$pval_threshold[rownum],
                                 filter_type = com_params$filter_type[rownum],cutoff_val = com_params$cutoff_val[rownum],DE_type = com_params$DE_type[rownum],n_mrk = com_params$n_mrk[rownum],beta_mtx = beta_mtx,
                                 sig_name = sig_name, Gene_group =com_params$Gene_group[rownum],B = B,frac_true = frac_true,
                                 include_minfi = com_params$include_minfi[rownum], dmet_list = dmet_list,probeInfo = probeInfo,
                                 sc_signal = sc_signal_oneside,cpg_pergene = com_params$cpg_pergene[rownum],fin_ncpg = com_params$fin_ncpg[rownum],cutoff_val2 = com_params$cutoff_val2[rownum])
    if(alg1only){
      gc()
      return(list(output = output,output_onesided = output_onesided))
    }else{
      output3 <- Multi_mrk3_new(DM_df = my_DM_df,eQTM_df = my_eQTM_df,DE_df = my_DE_df,padjust_method =padjust_method,
                                ct_ind = ct_ind,pval_threshold = com_params$pval_threshold[rownum],
                                filter_type = com_params$filter_type[rownum],cutoff_val = com_params$cutoff_val[rownum],DE_type = com_params$DE_type[rownum],n_mrk = com_params$n_mrk[rownum],beta_mtx = beta_mtx,
                                sig_name = sig_name, Gene_group =com_params$Gene_group[rownum],B = B,frac_true = frac_true,
                                include_minfi = com_params$include_minfi[rownum], dmet_list = dmet_list,probeInfo = probeInfo,
                                sc_signal = sc_signal_twosided,cpg_pergene = com_params$cpg_pergene[rownum],fin_ncpg = com_params$fin_ncpg[rownum] ,cutoff_val2 = com_params$cutoff_val2[rownum])
      output_onesided3 <- Multi_mrk3_new(DM_df = my_DM_df_onesided,eQTM_df = my_eQTM_df,DE_df = my_DE_df_onesided,padjust_method =padjust_method,
                                         ct_ind = ct_ind,pval_threshold = com_params$pval_threshold[rownum],
                                         filter_type = com_params$filter_type[rownum],cutoff_val = com_params$cutoff_val[rownum],DE_type = com_params$DE_type[rownum],n_mrk = com_params$n_mrk[rownum],beta_mtx = beta_mtx,
                                         sig_name = sig_name, Gene_group =com_params$Gene_group[rownum],B = B,frac_true = frac_true,
                                         include_minfi = com_params$include_minfi[rownum], dmet_list = dmet_list,probeInfo = probeInfo,
  ...
  print_text <- c("1st alg 2-sided","1st alg 1-sided","3rd alg 2-sided","3rd alg 1-sided")
  #p1list <- list()
  # plist <- list()
  # sub_res2list <- list()
  # sub_reslist <- list()
  ...
  #     i[[j]][[3]]
  #   })
  #   print(paste0(print_text[j],range(sub_reslist[[j]],na.rm = T)))
  ...
  #}
  return(list(res = res, com_params_sub = com_params_sub ))
